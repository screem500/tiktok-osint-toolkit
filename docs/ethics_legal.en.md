# ⚖️ Legal & Ethical Guidelines

**[النسخة العربية](ethics_legal.md)**

> **Read this file before using any tool in this project.**

---

## ✅ Legitimate Uses

This project is intended exclusively for:

- 🎓 **Learning & education**: understanding how browsers, the DOM, and JavaScript work.
- 🔬 **Academic research**: on public data, with a documented methodology.
- 📰 **Investigative journalism**: within professional and legal standards.
- 🛡️ **Authorized security research**: with written contracts and defined scope.
- 👤 **Your own data**: archiving content from your personal account.

---

## ❌ Prohibited Uses

- Harassing, stalking, or intimidating any person.
- Collecting data about minors or targeting them in any way.
- Bypassing any technical protection: passwords, private accounts, geo-blocks.
- Impersonating real people or creating fake accounts in their names.
- Republishing copyright-protected content without permission.
- Any use that violates local laws in your country or the subject's country.

---

## 📜 General Legal Considerations

### Platform Terms of Service
- Heavy automated scraping may **violate the Terms of Service** of TikTok and other platforms.
- This project's scripts run manually inside your browser at a moderate pace, but **compliance is your responsibility**.

### Data Protection Laws
- **Saudi PDPL** (Personal Data Protection Law) and the **Anti-Cyber Crime Law**: personal data — even if publicly accessible — is regulated in collection, processing, and publishing; unauthorized access to systems or accounts is a crime.
- **GDPR** (European Union): personal data — even if public — is regulated in collection, processing, and storage.
- Local laws in your country (cybercrime, privacy, defamation) also apply to OSINT activities.
- Practical rule: collect the **minimum necessary**, for a defined purpose, and delete what you don't need.

### In Digital Forensics
- Examine devices only with **legal authorization** (personal ownership, court order, or employer authorization under a published policy).
- Maintain the [chain of custody](digital_forensics.en.md) or the evidence loses its value.

---

## 🧭 The Golden Rule

> Before any action, ask yourself:
> 1. Do I have a **legal right** to collect this information?
> 2. Could I **justify every step before a judge or a regulator** if asked?
> 3. Is the purpose **legitimate and defined**, or just curiosity?
>
> If you hesitate on any answer — **stop and consult a legal professional**.

---

## 📢 Disclaimer

The contributors of this project accept no liability for misuse. The content is provided "as is" for educational purposes. Using these tools means you accept full legal responsibility for your own actions.
